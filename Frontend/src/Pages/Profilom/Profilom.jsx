...
const MentettMenhelyek = () => {
    ...
    useEffect(() => {
      const fetchKedvencek = async () => {
            try {
                const response = await fetch(`${apiUrl}/users/kedvencekadattal/${userId}`);
  ...
  return(
    ...
    <div className="lista">
        {kedvencek.map((kedvenc) => (
          <div key={kedvenc._id} className="bg-white shadow-lg rounded-xl p-6 border border-gray-200 w-72 text-center p-3">
            <img src={kedvenc.logo} className="logo fooldallogo" />
            <h2 className="text-xl font-bold text-gray-800 mb-2">{kedvenc.menhelyneve}</h2>
          </div>
        ))}
      </div>
        </section>
